...
my $DESeq_anl_fol = `mkdir $parent/../GeneCounts_for_DESeq`; 
my $par_dir;
my $sub_dir;
my $R1;
my $R2;
...
while (my $sub_folders = readdir($par_dir)) {
		...
		next unless $file=~/\.f*q*/i; ## reads fastq file as < .fastq || .fq || .fastq.gz >
         	...
         	close($sub_dir);
       		...
       		### Now print list of commands in to be executed for each sample_folder into output file ==> scripts_out ####
		###------------------------------------------------------------------------------------------------------####
		...
		print $outfile "samtools view -h $path/tophat_out_$sub_folders/sorted_bam_$sub_folders.bam >$path/tophat_out_$sub_folders/sorted_bam_$sub_folders.sam\n\n";
	 	...
	 	print $outfile "mv $path/tophat_out_$sub_folders/$sub_folders.gene_counts $parent/../DESeq_anl_fol\n\n";
...
################---------------------------------------------------------------------------------------------------------------------------------######
    my$split_command=`split -l 28  -d $scripts_out sub_job`;
